    ...
    module Blublu
      module Group
        class Client
          ...
          def inviteToJoin(token, requestTime, sign, usrId)
            send_inviteToJoin(token, requestTime, sign, usrId)
            return recv_inviteToJoin()
          end

          def send_inviteToJoin(token, requestTime, sign, usrId)
            send_message('inviteToJoin', InviteToJoin_args, :token => token, :requestTime => requestTime, :sign => sign, :usrId => usrId)
          end
          ...
          def passInviteOrJoin(token, requestTime, sign, resouceId)
            send_passInviteOrJoin(token, requestTime, sign, resouceId)
            return recv_passInviteOrJoin()
          end

          def send_passInviteOrJoin(token, requestTime, sign, resouceId)
            send_message('passInviteOrJoin', PassInviteOrJoin_args, :token => token, :requestTime => requestTime, :sign => sign, :resouceId => resouceId)
          end

          def recv_passInviteOrJoin()
            result = receive_message(PassInviteOrJoin_result)
            return result.success unless result.success.nil?
            raise result.ouch unless result.ouch.nil?
            raise ::Thrift::ApplicationException.new(::Thrift::ApplicationException::MISSING_RESULT, 'passInviteOrJoin failed: unknown result')
          end

          def refuseInviteOrJoin(token, requestTime, sign, resouceId)
            send_refuseInviteOrJoin(token, requestTime, sign, resouceId)
            return recv_refuseInviteOrJoin()
          end

          def send_refuseInviteOrJoin(token, requestTime, sign, resouceId)
            send_message('refuseInviteOrJoin', RefuseInviteOrJoin_args, :token => token, :requestTime => requestTime, :sign => sign, :resouceId => resouceId)
          end

          def recv_refuseInviteOrJoin()
            result = receive_message(RefuseInviteOrJoin_result)
            return result.success unless result.success.nil?
            raise result.ouch unless result.ouch.nil?
            raise ::Thrift::ApplicationException.new(::Thrift::ApplicationException::MISSING_RESULT, 'refuseInviteOrJoin failed: unknown result')
          end
        ...
        class Processor
          ...
          def process_inviteToJoin(seqid, iprot, oprot)
            ...
            begin
              result.success = @handler.inviteToJoin(args.token, args.requestTime, args.sign, args.usrId)
            rescue Blublu::InvalidOperation => ouch
              result.ouch = ouch
            end
          ...
          def process_passInviteOrJoin(seqid, iprot, oprot)
            args = read_args(iprot, PassInviteOrJoin_args)
            result = PassInviteOrJoin_result.new()
            begin
              result.success = @handler.passInviteOrJoin(args.token, args.requestTime, args.sign, args.resouceId)
            rescue Blublu::InvalidOperation => ouch
              result.ouch = ouch
            end
            write_result(result, oprot, 'passInviteOrJoin', seqid)
          end

          def process_refuseInviteOrJoin(seqid, iprot, oprot)
            args = read_args(iprot, RefuseInviteOrJoin_args)
            result = RefuseInviteOrJoin_result.new()
            begin
              result.success = @handler.refuseInviteOrJoin(args.token, args.requestTime, args.sign, args.resouceId)
            rescue Blublu::InvalidOperation => ouch
              result.ouch = ouch
            end
            write_result(result, oprot, 'refuseInviteOrJoin', seqid)
          end
        ...
        class InviteToJoin_args
          ...
          FIELDS = {
            ...
            REQUESTTIME => {:type => ::Thrift::Types::I64, :name => 'requestTime'},
            SIGN => {:type => ::Thrift::Types::STRING, :name => 'sign'},
            USRID => {:type => ::Thrift::Types::I64, :name => 'usrId'}
          }
        ...
        class RefuseInviteOrJoin_result
          include ::Thrift::Struct, ::Thrift::Struct_Union
          SUCCESS = 0
          OUCH = 1
